...
{
	//HAL_SPI_Transmit(&hspi1, &data, 0x01, 10);  
...
void ST7735_DMA_Init(void)
{
	...
	ST7735_DMA_Channel->CCR |= (DMA_CCR_DIR | DMA_CCR_TCIE);
...
void ST7735_DMA_FillScreen(uint8_t *array,uint8_t increment_memory)
{
	...
	{
			ST7735_DMA_Channel->CCR&=~DMA_CCR_MINC;
	}
	else
	{
			ST7735_DMA_Channel->CCR|=DMA_CCR_MINC;
	...
	ST7735_DMA_Channel->CCR|=DMA_CCR_EN;
}
...
void ST7735_DMA_DrawBMP(uint8_t *array,uint8_t x,uint8_t y,uint8_t width,uint8_t height)
{
	...
	ST7735_DMA_Channel->CCR|=(DMA_CCR_EN|DMA_CCR_MINC);
}
